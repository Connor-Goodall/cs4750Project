...
function findPosts ($name)  // given a search for a club name, returns relevant info about all the posts created by that club
{
    ...
    $regex = $l.$name.$l;
    global $db;
    $query = "SELECT `Name`, `Title`, `Body_Text`, `Post_Date`, `Picture`, `Upvotes`, `Downvotes` FROM `post` NATURAL JOIN `club` 
    WHERE club.Name LIKE :Regex OR club.Nickname LIKE :Regex";
